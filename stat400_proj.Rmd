...
The response variable/parameter of interest is 'children', the number of living children.  We are trying to predict the number of living children using the explanatory variables 'age', 'urban', 'educ', 'agefm' and 'usemeth', and whether this is an effective model of prediction.  
...
```{r}
# select data based on our research question
selectData <- fertil2 %>%
...
From graph above, the age of mothers seems like to follow normal distribution even if there is a tail on the right side. The mean of mothers' age is around 30.
...
From the graph, we can see that there are two values (educ=0 and educ=7) in education appearing to be significant large, which means the most of women in Botswana have no education experience or just elementary education experience.
...
From the table and graph, we can see that the number of mothers who use birth control is about twice that of not; 1323 (p = 0.65) compared to 706 who do not (p = .35).
...
From the graph, the age of first marriage approximately follows normal distribution with mean at 20, and it seems to be a little right skewed.
...
By looking at the data, some of the graph have too few data to show its distribution, so we don't know if it will follow normal distribution as desired. From the table, we can see that some means and variance are not the same, which means the overdispersion might be a problem.
...
From residuals vs. fitted graph, we can see that the fitted line is about horizontal line around x axis, which means the assumption of linearity is met.
...
From the graphs and tables, the mean and variance is not eactly the same for some group of data. Therefore, we need to check the overdispersion problem for this model too.
...
According to the summary of two models above, we can see that the AIC value for model 2 is relatively low, so here we choose the model 2 as a better predictor for the number of children. We cannot use the drop-in-deviance method here because these two models are not nested.
...
Since mean and variance are not very equal as we shown before, the assumption may not very perfect in this. So we need to take the overdispersion into account. Since the value of two models' dispersion parameters are not noticeably greater or less than 1, we don't have to use the quasipoisson in this case. 
...
We have hypothesis: $H_0$: the model 2 fits, $H_a$: the model 2 does not fit. Since the p-value I get is 0.99, which is relatively high value, in that case we are to fail to reject the null hypothesis. In conclusion, according to the p-value we calculated, we say that the model 2 fits.
...
We have hypothesis: $H_0$: the model 1 fits, $H_a$: the model 1 does not fit. Since the p-value is 0, which is very small, in that case we have enough evidence to reject the null hypothesis. In conclusion, we say the model 1 does not fit.
...
### Question 1  
Is the model using parameters 'agefm', 'educ', and 'usemeth' to predict the response variable 'children' efficient?  

Model one displays this outlook. The model is not appropriate to predict the children response variable based on the goodness-of-fit test, as we find a p-value equal to zero and reject the notion that the model fits.

### Question 2  
Is the model using parameters 'age', 'educ', and 'urban' to predict the response variable 'children' efficient?  

Model two displays this outlook. This model is appropriate to predict the children response variable based on the goodness-of-fit (Chi-Squared) test, as we find a p-value nearly equal to one and fail to reject the notion that the model fits.
...
Research Question 2: Does time spent doing homework and the type of school (private or public) affect math scores?
...
The spread of the private vs public math scores and the number of students is about the same for both types of schools, with the median for private schools (median = 59) being noticeable higher than that for public (median = 47) schools, so students in private schools have better math scores overall. 

```{r}
plot <- ggplot(data = Multidata, mapping = aes(y = math, x = homework))+
  geom_point()+
  geom_smooth(method = "lm", se = F) +
  labs(x = "Time Spend on Homeworl",
       y = "Math score")
plot
```

We can assume linearity between amount of time spent on homework and math scores based on the graph above.  There is a clear positive linear trend displayed by the blue line. This means that the more time students spend on their homework, the higher scores the will get.
...
The answer to the second research question is simply no. The AIC for the model with homework and that for the model with homework and public have similar AIC values which means that they perform relativity similarly. Further, the goodness-of-fit test for this model does not give evidence that the model fits to predict the variable of interest, 'math'.
